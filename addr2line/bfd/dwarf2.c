	...
	unsigned int* bytes_read_ptr)
{
	...
	*bytes_read_ptr = unit->offset_size;
	...
	{
		bfd* debug_bfd;
		char* debug_filename = NULL;//!!! bfd_follow_gnu_debugaltlink(unit->abfd, DEBUGDIR);
	...
	bfd_boolean do_place)
{
	...
	*pinfo = stash;
	...
	{
		char* debug_filename = NULL;//!!! bfd_follow_gnu_debuglink(abfd, DEBUGDIR);
...
static bfd_boolean
enum_address_in_line_info_table(bfd* abfd, asection* section, struct comp_unit* unit, struct dwarf2_debug* stash, bfd_boolean trace)
{
	...
	void** pinfo)
{
	...
	{
		//struct funcinfo* function_ptr = 0; lookup_address_in_function_table(each, 0x5E4A0, &function_ptr);
		if (!enum_address_in_line_info_table(abfd, section, each, stash, TRUE))
			return FALSE;
	}
	...
	while (stash->info_ptr < stash->info_ptr_end)
	{
		...
		if (length > 0)
		{
			...
			if (!comp_unit_maybe_decode_line_info(each, stash))
				break;
