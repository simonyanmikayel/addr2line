  ...
  while (bfd_target_vector[t] != NULL)
    {
      ...
      wid = LONGEST_ARCH + (int)strlen (bfd_target_vector[t]->name) + 1;
      ++t;
      while (wid < columns && bfd_target_vector[t] != NULL)
	{
	  int newwid;

	  newwid = wid + (int)strlen (bfd_target_vector[t]->name) + 1;
	  if (newwid >= columns)
	    break;
	  wid = newwid;
	  ++t;
...
bfd_get_archive_filename (const bfd *abfd)
{
  ...
  if (needed > curr)
	...
	free (buf);
      ...
      buf = (char *) bfd_malloc ((bfd_size_type)curr);
